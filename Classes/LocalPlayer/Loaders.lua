...
function LocalPlayer:printTalentsInfo()
    if lastCastTime ~= 0 and DMW.Time - lastCastTime < 20  then
        return
    end
    lastCastTime = DMW.Time
    local Selected
    for i = 1, 90000 do
        local ActiveSpec = GetActiveSpecGroup()
        Selected = select(4, GetTalentInfoByID(i, ActiveSpec))
        if Selected then
            print("pvp", GetTalentInfoByID(i, ActiveSpec))
        end
        Selected = select(4, GetPvpTalentInfoByID(i, ActiveSpec))
        if Selected then
            print("pvp", GetPvpTalentInfoByID(i, ActiveSpec))
        end
    end
end
function LocalPlayer:GetTalents()
    ...
    for TalentName, TalentID in pairs(DMW.Enums.Spells[self.Class][self.Spec].Talents) do
        ...
        if Selected then
            ...
            Selected = select(4, GetPvpTalentInfoByID(TalentID, ActiveSpec))
            if Selected then
                self.Talents[TalentName] = {Active = true, Value = 1}
            else
...
function LocalPlayer:UpdateProfessions()
    ...
    if prof1 then
        local name, icon, skillLevel, maxSkillLevel, numAbilities, spelloffset, skillLine, skillModifier, specializationIndex, specializationOffset, curName = GetProfessionInfo(prof1)
        if curName == "库尔提拉斯剥皮" then
            self.Professions.Skinning = skillLevel
        end
        if curName == "库尔提拉斯采矿" then
            self.Professions.Mining = skillLevel
        end
        if curName == "库尔提拉斯草药学" then
            self.Professions.Herbalism = skillLevel
        end
        if curName == "库尔提拉斯钓鱼" then
            self.Professions.Fishing = skillLevel
        end
    end
    if prof2 then
        local name, icon, skillLevel, maxSkillLevel, numAbilities, spelloffset, skillLine, skillModifier, specializationIndex, specializationOffset, curName = GetProfessionInfo(prof2)
        if curName == "库尔提拉斯剥皮" then
            self.Professions.Skinning = skillLevel
        end
        if curName == "库尔提拉斯采矿" then

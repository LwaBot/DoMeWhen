...
local Status = CreateFrame("BUTTON", "DMWHUDStatusText", UIParent)
local Settings

local function DisplaySpellName(SpellName)
    if not SpellName then
        return
    end
    gui:SetText(SpellName)
end
function HUD.Init()
    Settings = DMW.Settings.profile
    HUDFrame:SetWidth(120)
    HUDFrame:SetHeight(200)
    HUDFrame:SetPoint(Settings.HUDPosition.point, UIParent, Settings.HUDPosition.relativePoint, Settings.HUDPosition.xOfs + 20, Settings.HUDPosition.yOfs)
...
local RotationSetting = {
    Rotation = {
        [1] = { Text = "RT |cFF00FF00Enabled", Tooltip = "" },
        [2] = { Text = "RT |cffff0000Disabled", Tooltip = "" }
    }
}

local DMW = DMW

DMW.Enums.Spells = {
    GLOBAL = {
        All = {
            Abilities = {
                GCD = {SpellID = 61304, CastType = "Special"},
                AegisOfTheDeep = {SpellID = 298168},
                AnimaOfDeath = {SpellID = 294926},
                AzerothsUndyingGift = {SpellID = 298081},
                BloodOfTheEnemy = {SpellID = 297108},
                ConcentratedFlame = {SpellID = 299349},
                EmpoweredNullBarrier = {SpellID = 295746},
                EngineOfCeaselessProgress = {SpellID = 295828},
                FocusedAzeriteBeam = {SpellID = 295258, CastType = "Ground"},
                GuardianOfAzeroth = {SpellID = 299355},
                LifeBindersInvocation = {SpellID = 299943},
                MemoryOfLucidDreams = {SpellID = 298357},
                NearDeath = {SpellID = 310690},
                OverchargeMana = {SpellID = 296072},
                PurifyingBlast = {SpellID = 299345},
                Refreshment = {SpellID = 296197},
                RippleInSpace = {SpellID = 302731},
                Standstill = {SpellID = 299882},
                SuppressingPulse = {SpellID = 300009},
                TheUnboundForce = {SpellID = 299376},
                VitalityConduit = {SpellID = 299958},
                WorldveinResonance = {SpellID = 295186},
                SpiritSalvation = {SpellID = 297375},
                ControlDispel = {SpellID = 208683},
                --Professions, multiple ID's shouldn't matter, just for tracking casts
                Mining = {SpellID = 2656, CastType = "Profession"},
                HerbGathering = {SpellID = 2366, CastType = "Profession"},
                Skinning = {SpellID = 8613, CastType = "Profession"}
            },
            Buffs = {
                MemoryOfLucidDreams = 298357,
                RecklessForce = 302932,
                SeethingRage = 297126,
            },
            Debuffs = {
                Escape = {SpellID = 26013},
            },
            Essences = {
                ArcaneTorrent = 202719,
                AegisOfTheDeep = 298168,
                AnimaOfDeath = 294926,
                AzerothsUndyingGift = 298081,
                BloodOfTheEnemy = 297108,
                ConcentratedFlame = 295373,
                CondensedLifeForce = 299357,
                EmpoweredNullBarrier = 295746,
                FocusedAzeriteBeam = 295258,
                GuardianOfAzeroth = 299355,
                HeartEssence = 296208,
                LifeBindersInvocation = 299943,
                MemoryOfLucidDreams = 298357,
                OverchargeMana = 296072,
                PurifyingBlast = 299345,
                Refreshment = 296197,
                RippleInSpace = 302731,
                Standstill = 299882,
                SuppressingPulse = 300009,
                TheUnboundForce = 299376,
                VitalityConduit = 299958,
                WorldveinResonance = 295186
            }
        }
    }
}

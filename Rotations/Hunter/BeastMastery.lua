...
local function SingleTarget()
    ...
    if (Spell.BarbedShot:Charges() < 2 or Buff.Frenzy:Stacks(Pet) > 2 or not Trait.PrimalInstincts.Active) then
...
local function Interrupt()
    if HUD.Interrupts == 1 then
        ...
        if Target and Target.ValidEnemy and not Target.Dead and Spell.Intimidation:Cast(Target) then
            return true
        end
        ...
        if Player40YC > 0 then
            for _, Unit in pairs(Player40Y) do
                if Unit:Interrupt() then
                    ...
                    if Spell.Intimidation:Cast(Unit) then
                        return true
                    end

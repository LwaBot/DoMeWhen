...
local function Dispel()
    for _, Friend in ipairs(Friends40Y) do
        if HUD.Dispel == 1 and Friend:Dispel(Spell.CleanseToxins) then
            Spell.CleanseToxins:Cast(Friend)
        end
...
local function Defensive()
    ...
    if Setting("Cleanse Toxins") and Spell.CleanseToxins:IsReady() then
        ...
        for _, Unit in pairs(Player40Y) do
            ...
            if Unit:ControlDispel(Spell.ControlDispel) and Spell.ControlDispel:Cast(Player) then
                return true
            end
        end
    end
